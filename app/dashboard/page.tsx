...
export default function DashboardPage() {
  ...
  const Card = ({ e }: { e: EventItem }) => {
    ...
    return (
      <div style={{ border: "1px solid #eee", borderRadius: 16, padding: 14, background: "#fff" }}>
        <div style={{ fontWeight: 900, fontSize: 15, marginBottom: 6 }}>🍕 {e.summary || "Commande"}</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
          <span style={badge}>{isDelivery ? "🚚 Livraison" : "🥡 À emporter"}</span>
          <span style={badge}>⏱ {formatDateTime(e.createdAt)}</span>
          {total > 0 && <span style={badge}>💰 {total}€</span>}
        </div>
        {e.customerName && <div style={{ fontSize: 13, marginBottom: 4 }}><b>Client :</b> {e.customerName}{e.customerPhone ? ` — ${e.customerPhone}` : ""}</div>}
        {e.data?.address && <div style={{ fontSize: 13, marginBottom: 8 }}><b>Adresse :</b> {e.data.address}</div>}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
          ...
          <button onClick={() => setStatus(e.id, "cancelled")} style={{ ...btn, color: "#c00", background: "#fff7f7", border: "1px solid #fdd" }}>✖ Annuler</button>
        </div>
      </div>
    );
  };
  ...
  const Column = ({ title, subtitle, list }: { title: string; subtitle?: string; list: EventItem[] }) => (
    <div style={{ border: "1px solid #eee", borderRadius: 18, padding: 14, background: "#fafafa", minHeight: 200 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 17, fontWeight: 900 }}>{title}</div>
          {subtitle && <div style={{ fontSize: 12, opacity: 0.6 }}>{subtitle}</div>}
        </div>
        <span style={badge}>{list.length}</span>
      </div>
      <div style={{ display: "grid", gap: 10 }}>
        {list.length === 0 ? <div style={{ opacity: 0.5 }}>Aucune commande</div> : list.map(e => <Card key={e.id} e={e} />)}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 900, margin: 0 }}>📋 {businessName}</h1>
          <p style={{ fontSize: 12, color: "#999", margin: "4px 0 0" }}>Temps réel</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {newAlert && <div style={{ background: "#ff4444", color: "#fff", padding: "8px 14px", borderRadius: 10, fontSize: 13, fontWeight: 800 }}>🔔 Nouvelle commande !</div>}
...
const badge: React.CSSProperties = { display: "inline-block", fontSize: 12, padding: "3px 8px", borderRadius: 99, border: "1px solid #eee", background: "#fff" };
const btn: React.CSSProperties = { padding: "8px 12px", borderRadius: 10, border: "1px solid #eee", background: "#fafafa", fontWeight: 700, cursor: "pointer", fontSize: 13 };
